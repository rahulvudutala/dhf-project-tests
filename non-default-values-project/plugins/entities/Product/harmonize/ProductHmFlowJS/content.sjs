...
function extractInstanceProduct(source) {
  ...
  let ProductID = !fn.empty(fn.head(source.xpath('/ProductID'))) ? xs.integer(fn.head(fn.head(source.xpath('/ProductID')))) : null;
  let ProductName = !fn.empty(fn.head(source.xpath('/ProductName'))) ? xs.string(fn.head(fn.head(source.xpath('/ProductName')))) : null;
  let UnitPrice = !fn.empty(fn.head(source.xpath('/UnitPrice'))) ? xs.double(fn.head(fn.head(source.xpath('/UnitPrice')))) : null;
  let SupplierID = !fn.empty(fn.head(source.xpath('/SupplierID'))) ? xs.integer(fn.head(fn.head(source.xpath('/SupplierID')))) : null;
